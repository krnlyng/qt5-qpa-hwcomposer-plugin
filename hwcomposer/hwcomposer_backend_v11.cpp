...
{
    qCDebug(QPA_LOG_HWC, "(HWCT) action: %d", e->type());
	...
	if(false) {
        ...
        if (backend->hwcVersion() == HWC_DEVICE_API_VERSION_1_4) {
            HWC_PLUGIN_EXPECT_ZERO(hwcDevice->setPowerMode(hwcDevice, 0, HWC_POWER_MODE_NORMAL));
        } else
#endif
#ifdef HWC_DEVICE_API_VERSION_1_5
        if (backend->hwcVersion() == HWC_DEVICE_API_VERSION_1_5) {
            HWC_PLUGIN_EXPECT_ZERO(hwcDevice->setPowerMode(hwcDevice, 0, HWC_POWER_MODE_NORMAL));
        } else
#endif
...
#ifdef HWC_DEVICE_API_VERSION_1_4
        if (backend->hwcVersion() == HWC_DEVICE_API_VERSION_1_4) {
            HWC_PLUGIN_EXPECT_ZERO(hwcDevice->setPowerMode(hwcDevice, 0, HWC_POWER_MODE_OFF));
        } else
#endif
#ifdef HWC_DEVICE_API_VERSION_1_5
        if (backend->hwcVersion() == HWC_DEVICE_API_VERSION_1_5) {
